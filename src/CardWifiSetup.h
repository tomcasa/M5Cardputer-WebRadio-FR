...
void connectToWiFi() {
    ...
    //CFG_WIFI_SSID = preferences.getString(NVS_SSID_KEY, "SSID hardcode ");
    //CFG_WIFI_PASS = preferences.getString(NVS_PASS_KEY, "wifi pass");
    preferences.end();
    ...
    M5Cardputer.Display.print(CFG_WIFI_SSID);
    ...
    while (tm++ < 110 && WiFi.status() != WL_CONNECTED) {
        ...
        if (M5Cardputer.BtnA.isPressed()){
                M5Cardputer.Speaker.tone(440, 1000);
                Preferences preferences;
                preferences.begin("wifi_settings", false);
                preferences.clear();
                preferences.end();
